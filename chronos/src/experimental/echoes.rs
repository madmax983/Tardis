...
mod tests {
    ...
    async fn test_echoes() {
        ...
        vortex.set_mock_load_model(|_, _| Ok(ModelHandle::new(0)));
        vortex.set_mock_embedding(|_| Ok(vec![0.1, 0.2, 0.3]));
        // Inference not used here
        ...
        // Setup Conversation
        let session_id = SessionId::new(); // Just a dummy session ID
        let msg = Message {
            id: EntityId::new(),
            session_id,
        ...
        gallifrey.conversation().add_message(msg).unwrap();
